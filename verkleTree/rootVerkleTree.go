...
type rootMembershipProof struct {
	Witnesses   []rootWitnessStruct
	Commitments []e.G1
}

// Struct representing the verkle-tree
type rootVerkleTree struct {
	Root   *rootNode
	leafs  []*rootNode
	fanOut int
	pk     pubParams
...
func rootBuildTree(certs [][]byte, fanOut int, pk pubParams, numThreads ...int) *rootVerkleTree {
	...
	for len(nextLayer) > 1 {
		...
		var nodesForThread []*rootNode
		nextLayer2 := make([][]*rootNode, numThreads[0])
		var mu sync.Mutex
...
func rootMakeLayer(nodes []*rootNode, fanOut int, firstLayer bool, pk pubParams, index int, nextlayerPointer *[][]*rootNode, mu *sync.Mutex) []*rootNode {
	...
	nextLayer := make([]*rootNode, len(nodes)/fanOut) // divided with fanout which is length of vectors
	...
	for i := 0; i < len(nodes); {
		...
		childrenList := make([]*rootNode, fanOut)
		...
		//start = time.Now()
		commitment := rootCommit(pk, vectToCommit)
		//elapsed = time.Since(start)
...
func rootVerifyNode(cert []byte, tree rootVerkleTree) bool {
	mp := rootCreateMembershipProof(cert, tree)
	return rootVerifyMembershipProof(mp, tree.pk)
}
...
func rootCreateMembershipProof(cert []byte, tree rootVerkleTree) rootMembershipProof {
	var nod *rootNode
	...
	var witnessList []rootWitnessStruct
	var commitList []e.G1
	if notInList {
		return rootMembershipProof{}
	}
	nod = tree.leafs[n]
	//Creates the lists required for membership proof
	witnessStructEmpty := rootWitnessStruct{}
	...
	return rootMembershipProof{Witnesses: witnessList, Commitments: commitList}
}
...
func rootVerifyMembershipProof(mp rootMembershipProof, pk pubParams) bool {
